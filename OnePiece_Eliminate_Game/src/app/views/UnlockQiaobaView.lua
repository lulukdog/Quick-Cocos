...
local UnlockQiaobaView = class("UnlockQiaobaView", function()
    return display.newNode()
end)

function UnlockConfirm_qiaoba( _result )
    print("UnlockConfirm_qiaoba ".._result)
    ...
    if resultCfg[1] ~= "fail" then
        ...
        sendMessage({msg="MapScene_BuyHelperSuccess",helperNum=6})
    else
...
function UnlockQiaobaView:ctor()

	self._mainNode = CsbContainer:createPushCsb("UnlockQiaobaView.csb"):addTo(self)
    local _ani = cc.CSLoader:createTimeline("UnlockQiaobaView.csb")
	...
	CsbContainer:decorateBtn(closeBtn,function()
        scheduler.unscheduleGlobal(self.schedulerF)
        scheduler.unscheduleGlobal(self.schedulerS)
    ...
    CsbContainer:decorateBtn(confirmBtn,function()
        if game.helper[6]==0 then

            common:javaOnUseMoney(UnlockConfirm_qiaoba,3000)
    ...
    self:addLightSweep("mTitle3Node","mTitle3Sprite",3)
    self:addLightSweep("mTitle4Node","mTitle4Sprite",4)
end

function UnlockQiaobaView:addLightSweep(nodeName,spriteName,tag)
    ...
    if tag==2 then
        self.schedulerS = scheduler.performWithDelayGlobal(function()
    ...
    elseif tag==4 then
        self.schedulerF = scheduler.performWithDelayGlobal(function()
            spark:runAction(cc.RepeatForever:create(cc.Sequence:create(
                cc.CallFunc:create(function()
                    spark:setPositionX(-_logoSprite:getContentSize().width)
                end),
